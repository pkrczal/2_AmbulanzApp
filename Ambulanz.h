/**
(c) Hochschule Heilbronn
Fakultaet fuer Informatik 
Prof. Dr. R. Bendl


Einfuehrung in C++ (Medizinische Informatik Bachelor)

Rudimentaere Rumpfimplementierung einer Applikation
zur Demonstration grundlegender C++ Moeglichkeiten. 

Aus didaktischen Gruenden (reduzierte Komplexitaet
fuer Einsteiger) wurden nicht alle Features optimal 
implementiert. Die Module koennen sukzessive von Studierenden
erweitert und verbessert werden

Klasse Ambulanz

Verwaltet die verschiedenen Datenobjekte in der Ambulanz
einer Klinik (Patienten, Aerzte ...) zur Terminplanung,
Dokumentation der Leistungen ...

Maintenance Info:
2017-09-25 R. Bendl
Modul erstellt

*/
#pragma once

#include "PatientenAdmin.h"
//#include "AerzteAdmin.h"

class Ambulanz
 {
  public:
    Ambulanz ()
     {
      patAdmin = new PatientenAdmin();
      patAdmin->start();
     }

   PatientenAdmin* getPatientAdmin() { return patAdmin;}
   ~Ambulanz() 
     {
      if(patAdmin) delete patAdmin;
     }
   private:
     PatientenAdmin* patAdmin;
//    AerzteAdmin* aerzte;
 };
